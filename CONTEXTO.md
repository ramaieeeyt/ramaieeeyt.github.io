...
| El `clip-path` de la animación del hero **sobra por arriba y por abajo** | `slide-out` terminaba en `inset(0 -.15em 0 0)`, y ese `0` corta justo en la caja de línea. La caja de línea es **más baja que las letras**: a 73 px con `line-height:1.1` el renglón mide 81 px y la tipografía necesita 100, así que las colas de las «y» de «Yachay» y «University» salían cortadas en plano. Los valores negativos ensanchan el recorte. **Si tocas el `line-height` del hero, revisa que la holgura siga bastando** |
